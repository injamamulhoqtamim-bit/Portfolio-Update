    ...
    <footer className="relative bg-dark3 border-t border-border pt-12 pb-8 px-[5%] overflow-hidden">
      ...
      <div className="relative z-20 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr] gap-10 md:gap-8 mb-10 text-center sm:text-left">
        ...
        <div className="flex flex-col items-center sm:items-start col-span-1 sm:col-span-2 lg:col-span-1">
          ...
          <p className="text-[0.85rem] md:text-[0.82rem] text-muted leading-[1.65] max-w-[280px] sm:max-w-[340px] lg:max-w-[240px]">
          ...
          <div className="flex gap-4 mt-5 sm:mt-4">
            {[
              ...
              <a key={i} href={social.url} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-cyan transition-colors duration-300 transform hover:scale-110">
                {social.icon}
              </a>
            ))}
          </div>
        ...
        <div>
          ...
          <ul className="list-none p-0 m-0 flex flex-col gap-2.5">
            {["home", "about", "skills", "projects", "contact"].map(link => (
              <li key={link}>
                <a href={`#${link}`} className="text-muted text-[0.85rem] md:text-[0.82rem] no-underline transition-colors duration-300 capitalize hover:text-cyan">{link}</a>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="list-none p-0 m-0 flex flex-col gap-2.5">
            ...
            <li><a href="tel:+8801511994008" className="text-muted text-[0.85rem] md:text-[0.82rem] no-underline transition-colors hover:text-cyan">+880 1511-994008</a></li>
            <li><span className="text-muted text-[0.85rem] md:text-[0.82rem]">Dhaka, Bangladesh</span></li>
            <li className="mt-1 flex justify-center sm:justify-start">
              <a href="#contact" className="text-cyan text-[0.85rem] md:text-[0.82rem] no-underline flex items-center gap-2">
                <span className="w-2 h-2 bg-[#00ff88] rounded-full inline-block animate-ping"></span>
                Available for work
              </a>
            </li>
          </ul>
