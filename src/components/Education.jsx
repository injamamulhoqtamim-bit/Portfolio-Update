...
export default function Education() {
  ...
  // Filter education and experience items separately
  const educationItems = education.filter(
    (item) => item.type === "education" || item.type === "course" || !item.type
  );
  const experienceItems = education.filter(
    (item) => item.type === "experience" || item.type === "research"
  );
    ...
    const CardContent = (
      <div className="relative pl-8 pb-10 group last:pb-0">
        ...
        <div className="group relative overflow-hidden rounded-[16px] border border-border bg-card p-6 transition-all duration-300 hover:border-[rgba(0,212,255,0.3)]">
          ...
          {/* Institution */}
          <p className="mb-3 flex items-center gap-1.5 text-[0.9rem] font-bold text-cyan2">
            {item.institution}
            {item.link && (
